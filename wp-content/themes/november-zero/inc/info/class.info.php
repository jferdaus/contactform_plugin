...
if ( ! class_exists( 'November_Zero_Info' ) ) {
	...
	class November_Zero_Info {
		 ...
		 * Single instance.
		 *
		 * @var November_Zero_Info $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof November_Zero_Info ) ) {
				self::$instance = new November_Zero_Info;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s Info', 'november-zero' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s Info', 'november-zero' ), $this->theme_name );
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_november_zero_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_november_zero_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_page() {
			?>
			<div class="wrap about-wrap wt-info-wrap">
				...
				<?php endif; ?>
				<a href="https://novemberthemes.com" target="_blank"><div class="wp-badge"></div></a>
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'november-zero' ), $method );
			}
		}
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section wt-section wt-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .wt-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section wt-section wt-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .wt-section -->
		...
		public function useful_plugins() {
			...
			?>
			<div class="wt-section wt-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'november-zero' ); ?></a>
				<?php endif; ?>
		...
		public function demo_content() {
			...
			?>
			<div class="wt-section wt-section-demo-content">
		...
		public function upgrade_to_pro() {
			...
			?>
			<div class="wt-section wt-section-upgrade-to-pro">
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'november-zero-info', get_template_directory_uri() . '/inc/info/css/info' . $min . '.css', array(), '1.0.0' );
				wp_enqueue_script( 'november-zero-info', get_template_directory_uri() . '/inc/info/js/info' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'november_zero_dismiss_status', true );
			?>
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'november_zero_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'november_zero_dismiss_status', 1 );
